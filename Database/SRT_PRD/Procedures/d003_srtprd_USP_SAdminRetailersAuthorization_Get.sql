...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_SAdminRetailersAuthorization_Get` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_SAdminRetailersAuthorization_Get`(
	...
	IF (var_Method_Name = 'Get') THEN
		BEGIN
			...
			SELECT 
                ...
                retailer.Approved_Id, 
                retailer.Approved_Name, 
                retailer.Approval_Remarks,
                DATE_FORMAT(retailer.Approved_On, '%d %M %Y') AS Approved_On
			...
			AND (CAST(retailer.Created_On AS DATE) >= var_StartDate 
			AND CAST(retailer.Created_On AS DATE) <= var_EndDate)
			AND retailer.SalesUser_Id LIKE var_SalesUser_Id
	...
	ELSEIF (var_Method_Name = 'Get_One') THEN
		BEGIN
			SELECT Retailer_Id, Retailer_Name, 
                ...
                Is_Approved, Approved_Id, Approved_Name, Approval_Remarks,
                DATE_FORMAT(Approved_On, '%d %M %Y') AS Approved_On
            FROM mu09_retailer 
