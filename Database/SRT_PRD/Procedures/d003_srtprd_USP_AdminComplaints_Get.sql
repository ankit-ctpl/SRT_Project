...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AdminComplaints_Get` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AdminComplaints_Get`(
...
BEGIN
    ...
    BEGIN
		...
		SELECT complaint.Org_Id, 
			...
			ctype.ComplaintType_Id, 
            ctype.ComplaintType_Name,
            ...
            IFNULL(
				CASE complaint.Complaint_For
					WHEN 'farmer' THEN farmer.Farmer_Id
                    WHEN 'agent' THEN agent.Agent_Id
			...
			cstatus.ComplaintStatus_Id, 
            cstatus.ComplaintStatus_Name,
            IFNULL(
				CASE complaint.ComplaintStatus_Id
                WHEN ('C035004' || 'C035005') THEN 1
                ELSE 0
                END, '') AS Is_Closed
		FROM t016_complaint_header complaint
		...
		LEFT JOIN mu04_farmer farmer
			ON complaint.Complaint_For = 'farmer'
            AND (farmer.Farmer_Id = complaint.Complaint_For_User_Id)
			-- OR farmer.Farmer_Id = complaint.Complaint_By_User_Id)
            AND farmer.Org_Id = complaint.Org_Id
		LEFT JOIN mu05_agent agent
			ON complaint.Complaint_For = 'agent'
            AND (agent.Agent_Id = complaint.Complaint_For_User_Id)
            -- OR agent.Agent_Id = complaint.Complaint_By_User_Id)
            AND agent.Org_Id = complaint.Org_Id
    ...
    BEGIN
		SELECT complaint.Org_Id, complaint.Complaint_Id, 
            ...
            -- complaint.Current_Status_Id, cstatus.ComplaintStatus_Name AS Current_Status_Name
		FROM t016_complaint_item complaint
        LEFT JOIN c035_complaintstatus cstatus 
			ON complaint.New_Status_Id = cstatus.ComplaintStatus_Id
			-- OR complaint.Current_Status_Id = cstatus.ComplaintStatus_Id)
