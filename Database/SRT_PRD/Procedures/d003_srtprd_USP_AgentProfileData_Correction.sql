...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AgentProfileData_Correction` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AgentProfileData_Correction`(
...
BEGIN
  ...
  if (var_Method_Name = 'ChangeDetails') then
    ...
    AND  Is_Approved = 0 and Request_Type =  Var_Request_Type  ) THEN 
        ...
        (var_Org_Id , @Request_Id , Var_MCC_Id , 'Agent' , var_Profile_Id , 'Agent' , var_Profile_Id , Var_Request_Type , Var_Request_Data,
        @Current_Datetime , 0
        );
