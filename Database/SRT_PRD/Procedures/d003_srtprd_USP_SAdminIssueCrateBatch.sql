...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_SAdminIssueCrateBatch` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_SAdminIssueCrateBatch`(
...
BEGIN
	...
	CALL USP_Number_Range ('t039_dispatch_crate_batch', @Year_Id, 'T039', '', @New_Id );

    insert into t039_dispatch_crate_batch (Org_Id, Dispatch_Id, Dealer_Code, Dealer_Name,
    Dispatch_Date, Quantity, Material_Code, Invoice_Number, Created_On ,Batch)  
