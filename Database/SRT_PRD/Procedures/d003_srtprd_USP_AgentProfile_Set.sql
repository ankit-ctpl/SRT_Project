...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AgentProfile_Set` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AgentProfile_Set`(
...
BEGIN
		...
		if (var_Method_Name = 'UpdateAgentXML') then
		begin
			Declare RowCnt int;
			...
			While var_CursorTestID <= RowCnt Do
				...
				if isnull(@var_Field_Name) = 0 then 
						if (@var_Field_Name = 'Birth_Date' ) then
							SET @var_SQL = concat('Update mu05_agent set ', @var_Field_Name , ' = STR_TO_DATE(@var_Field_Value, ''%m/%d/%Y'') where Org_Id = @var_Org_Id and Agent_Id = @var_Profile_Id');
							PREPARE dynamic_statement FROM @var_SQL;
							EXECUTE dynamic_statement;
							DEALLOCATE PREPARE dynamic_statement;
						else
						   SET @var_SQL = concat('Update mu05_agent set ', @var_Field_Name , ' = @var_Field_Value where Org_Id = @var_Org_Id and Agent_Id = @var_Profile_Id');
						   PREPARE dynamic_statement FROM @var_SQL;
						   EXECUTE dynamic_statement;
						   DEALLOCATE PREPARE dynamic_statement;
						end if;
			...
			select -1 as Result_Id, 
			'Not Saved' as Result_Description,
			var_Profile_Id as Result_Extra_Key ;
		end if ;
		end;
	end if;
