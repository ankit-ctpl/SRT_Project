...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_FarmerAdvance_Set` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_FarmerAdvance_Set`(
...
BEGIN
	if (var_Method_Name = 'Create') then
		begin
			...
			var_MCC_Id,'farmer',var_Profile_Id,'farmer',var_Profile_Id,Current_Datetime , 0); 
