...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_SAdminProduct_Get` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_SAdminProduct_Get`(
...
if (var_Method_Name = 'Get') then  
	...
	elseif (var_Method_Name = 'Get_One') then
		begin
			select Org_Id,Product_Id, Product_Name, Product_Code,
            Product_Group,Rate,Image as product_photo ,Is_Active,Is_Deleted
            from m017_product 
