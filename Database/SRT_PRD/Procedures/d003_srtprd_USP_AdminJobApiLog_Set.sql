...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AdminJobApiLog_Set` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AdminJobApiLog_Set`(
	...
	SET SESSION sql_require_primary_key = 0;
		if (var_Method_Name = 'Create') then
			begin
				...
				Call USP_Number_Range ('l002_joblog', Year_Id, 'L002', '', New_Entry_Id );
                
                Insert Into l002_joblog
