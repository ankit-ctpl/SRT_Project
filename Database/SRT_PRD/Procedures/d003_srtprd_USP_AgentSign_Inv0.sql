...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AgentSign_Inv0` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AgentSign_Inv0`(
	...
	SET SQL_SAFE_UPDATES=0;
		...
		if(var_Method_Name = 'SignIn') then
			...
			if exists (select 1 from mu11_user_deviceid where Org_Id = var_Org_Id and User_Id = @Agent_Id and User_Type = 'Agent' 
				 ...
				 update mu11_user_deviceid set LastEdited_On =  @Current_Datetime 
				...
				insert into mu11_user_deviceid (Org_Id,User_Id , User_Type , Device_Id , LastEdited_On  ) values
				 (var_Org_Id , @Agent_Id, 'Agent' , Var_Device_Id , @Current_Datetime) ;
      ...
      elseIF(var_Method_Name = 'SilentSignIn') then
		...
		if exists (select 1 from mu11_user_deviceid where Org_Id = var_Org_Id and User_Id = var_Profile_Id and User_Type = 'Agent' 
				...
				and Device_Id = Var_Device_Id;
				 ...
				 insert into mu11_user_deviceid (Org_Id,User_Id , User_Type , Device_Id , LastEdited_On  ) values
				(var_Org_Id , var_Profile_Id, 'Agent' , Var_Device_Id , @Current_Datetime) ;
