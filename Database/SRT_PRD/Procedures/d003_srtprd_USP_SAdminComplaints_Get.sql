...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_SAdminComplaints_Get` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_SAdminComplaints_Get`(
...
BEGIN
    ...
    BEGIN
		...
		SELECT complaint.Org_Id, 
            ...
            IFNULL(
                ...
                ELSE 0
                END, '') AS Is_Closed,
                ifnull(complaint.QualityNotification,'')  as qualitynotification	
		...
		LEFT JOIN mu09_retailer retailer
			ON retailer.Retailer_Id = complaint.Complaint_For_User_Id
            AND retailer.Org_Id = complaint.Org_Id
    ...
    BEGIN
        ...
        order by complaint.Entry_Id desc;
    
    END;
	ELSEIF(var_Method_Name = 'Get_Images') THEN
    BEGIN
		select concat('https://appdoc.srthoratmilk.in', Photo) as link
        from t037_sales_complaint_images 
        WHERE Org_Id = var_Org_Id
        and Photo is not null
        AND Complaint_Id = var_Complaint_Id;
    end;
    END IF;
END ;;
DELIMITER ;
