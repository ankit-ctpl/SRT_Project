...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AgentMilkCollection` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AgentMilkCollection`(
...
set @Current_Datetime = (SELECT CONVERT_TZ(NOW(), '+00:00', '+00:00'));
	...
	if (Var_Method_Name = 'CollectMilk') then
		...
		if(select 1 from t005_milkcollectionfarmer where Org_Id = Var_Org_Id and MCC_Id = Var_MCC_Id 
			...
			Set @Milk_Base_Rate = CAST(@Milk_Base_Rate AS DECIMAL(8, 2));
				...
				if (Var_Milk_Status = 'C016002')then 
					...
					INSERT INTO t005_milkcollectionfarmer ( Org_Id , FarmerCollection_Id , MCC_Id , MCCCollectionShift_Id , Farmer_Id,
		...
		IF(Var_Milk_SNF > @Milk_Base_SNF) THEN
                ...
                set @Collection_Date =  (select CAST(now() AS CHAR));
				set @Total_Milk_Rate =  GetMilkRateDate(Var_Org_Id, Var_MCC_Id, Var_ShiftType_Id, Var_Milk_FAT, Var_Milk_SNF, Var_Milk_Type_Id , @Collection_Date);

                
                if(ifnull(@Total_Milk_Rate, 0) in ( 0 , '') ) then
				...
				elseif(@MusterType = 10) then 
					...
					if (@Mcc_Type in ( 'C014002' , 'C014003') and Var_Milk_Status = 'C016001' )   then 
						...
						Set @Avgfat=  (select sum(Quantity_Ltr * Fat) / (sum(Quantity_Ltr)) from t005_milkcollectionfarmer
						...
						Set @Avgsnf=  (select sum(Quantity_Ltr * SNF) / (sum(Quantity_Ltr)) from t005_milkcollectionfarmer
						T005 inner join t004_mcccollectionshift T004 on T005.Org_Id = T004.Org_Id and T005.MCCCollectionShift_Id = T004.MCCCollectionShift_Id
						where T005.MCC_Id = Var_MCC_Id and T005.MCCCollectionShift_Id = Var_MCC_Collection_Shift  and 
						T005.MilkType_Id = Var_Milk_Type_Id and T005.MilkStatus_Id = 'C016001'  and T005.Is_Active = 1);
	...
	elseif(Var_Method_Name = 'GetFarmerStatus') then 
        ...
        if exists(select 1 from t005_milkcollectionfarmer where Org_Id = Var_Org_Id and MCC_Id = Var_MCC_Id 
			and MCCCollectionShift_Id = Var_MCC_Collection_Shift and Farmer_Id = Var_Farmer_Id  and MilkType_Id = Var_Milk_Type_Id ) then 
			...
			select 1 as Result_Id, 'Valid' as Result_Description, '' as Result_Extra_Key;  
        	
		end if;
	elseif(Var_Method_Name = 'GetFarmerOfflineStatus') then 
		
        if exists(select 1 from t103_milkcollectionfarmer_offline where Org_Id = Var_Org_Id and MCC_Id = Var_MCC_Id 
			and MCCCollectionShift_Id = Var_MCC_Collection_Shift and Farmer_Id = Var_Farmer_Id  and MilkType_Id = Var_Milk_Type_Id ) then 
		...
		elseif (Var_Method_Name = 'CollectMilkOffline') then
