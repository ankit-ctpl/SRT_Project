...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_AdminManageFarmerIncentiveSchemes_Get` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_AdminManageFarmerIncentiveSchemes_Get`(
...
BEGIN
    ...
    BEGIN
		...
		WHERE incentive.Is_For_Farmer = 1
    ...
    BEGIN
		SELECT incentive.Org_Id, incentive.IncentiveScheme_Id, incentive.Scheme_Name, 
            ...
            DATE_FORMAT(incentive.From_Date,'%d %M %Y') AS Formatted_From_Date, 
            DATE_FORMAT(incentive.To_Date,'%d %M %Y') AS Formatted_To_Date, 
            DATE_FORMAT(incentive.From_Date,'%Y-%m-%dT%H:%i') AS From_Date, 
            DATE_FORMAT(incentive.To_Date,'%Y-%m-%dT%H:%i') AS To_Date, 
    ...
    END;
    ELSEIF(var_Method_Name = 'Get_Farmers') THEN
    BEGIN
		SELECT farmer.farmer_id, farmer.farmer_code, farmer.farmer_name,
			'Eligible' AS Eligibility
        FROM t017_incentives_request request
        LEFT JOIN mu04_farmer farmer
			ON request.Request_For_User_Id = farmer.Farmer_Id
            AND request.Org_Id = farmer.Org_Id
		WHERE request.Request_For = 'farmer'
        AND request.Org_Id = var_Org_Id
        AND request.IncentiveScheme_Id = var_IncentiveScheme_Id
        AND request.Is_Approved = 1
        AND farmer.Is_Deleted = 0;
    END;
    END IF;
