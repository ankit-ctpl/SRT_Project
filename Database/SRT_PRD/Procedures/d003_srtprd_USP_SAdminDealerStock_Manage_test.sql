...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_SAdminDealerStock_Manage_test` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_SAdminDealerStock_Manage_test`(
    var_Quantity int
)
proc_Exit: BEGIN
    ...
    set sql_mode = '';
            ...
            order by Created_On asc limit 1);
             ...
             set @lastdate  = date('2024-05-02 10:56:22');
			...
			insert into temp(text) value(concat('Update Done crate in ->' , now()));
			...
			insert into temp(text) value(concat('update done crate out-> ' , now()));
            ...
            set @updatedate  = date('2024-02-01 10:56:22');
