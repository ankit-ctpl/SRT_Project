...
DELIMITER ;;
DROP PROCEDURE IF EXISTS `USP_FarmerOrder` ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `USP_FarmerOrder`(
...
BEGIN
  ...
  set @MCC_Id = (select MCC_Id from mu04_farmer where Org_Id = var_Org_Id and Farmer_Id = var_Profile_Id );
        ...
        ) values (Var_Org_Id , @Order_Id , 'Farmer',Var_Profile_Id, 'Farmer' , Var_Profile_Id , @Current_Datetime, 0 , @Total_Items, @Total_Price , 1 ,  @Current_Datetime , Var_Profile_Id , @MCC_Id , 'Product' ) ;
