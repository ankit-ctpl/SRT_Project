...
BEGIN
        ...
        -- select @Milk_Base_Rate;
			...
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date  FROM f002_milk_rate_current where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012002' and chart_id = @ChartId and Item_Applicable_Date <=  @Current_Datetime order by Item_Applicable_Date desc  ) slabs on 
			...
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date   FROM f002_milk_rate_current where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012003' and chart_id = @ChartId  and Item_Applicable_Date <=  @Current_Datetime order by Item_Applicable_Date desc ) slabs on 
			m014.Org_Id = slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			...
			(SELECT Org_Id , Slab_Id ,Amount, Item_Applicable_Date FROM f002_milk_rate_current where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012004' and chart_id = @ChartId  and Item_Applicable_Date <=  @Current_Datetime order by Item_Applicable_Date desc ) slabs on 
			m014.Org_Id =  slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			...
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date FROM f002_milk_rate_current where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012005' and chart_id = @ChartId  and Item_Applicable_Date <= @Current_Datetime order by Item_Applicable_Date desc  ) slabs on 
			m014.Org_Id = slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			where Slab_Min <= var_SNF and Slab_Max >= var_SNF order by slabs.Item_Applicable_Date desc limit 1;
        ...
        /*
        IF(var_Fat > @Milk_Base_FAT) THEN
			if (@Milk_Highfat IS NULL OR @Milk_Highfat in (null , '') )then
				RETURN 0;
			end if;
		end if;
        */
		...
		/*
		IF(var_SNF > @Milk_Base_SNF) THEN
			if( @Milk_HighSnf IS NULL OR  @Milk_HighSnf IN ( '' , NULL) ) then 
				RETURN 0  ;
			end if;
		end if;
