...
BEGIN
    ...
    SET @Chart_Id = (
        SELECT m001.Chart_Id 
        FROM m001_milk_rate_offline_mcc_header m001
        inner join m001_milk_rate_offline_mcc_farmer m001i on
        m001.Org_Id = m001i.Org_Id
        and m001.Chart_Id = m001i.Chart_Id
        and m001.MCC_Id = m001i.MCC_Id
        and m001i.Farmer_Id = var_farmer_id
        WHERE m001.org_id = var_org_id  
          AND m001.MCC_Id = var_mcc_id 
          AND m001.MilkType_Id = var_milk_type_id
          AND DATE(m001.Created_On) <= DATE(var_date) 
        ORDER BY m001.Created_On DESC 
        LIMIT 1
    );
    ...
    /*
    		set @Chart_Id = (SELECT m001.Chart_Id 
        FROM m001_milk_rate_offline_mcc_header m001
        inner join m001_milk_rate_offline_mcc_farmer m001i on
        m001.Org_Id = m001i.Org_Id
        and m001.Chart_Id = m001i.Chart_Id
...
*/
		...
		where Chart_Id = @Chart_Id and org_id = var_org_id  and ifnull(@Chart_Id,'')<>'' limit 1;
    
    
		select max(Slab_Min) into @Slab_Max_Fat from m001_milk_rate_offline_mcc_slab 
		where Chart_Id = @Chart_Id  and org_id = var_org_id and Slab_Type = 'FAT'  and ifnull(@Chart_Id,'')<>''  limit 1;
    
		
        select max(Slab_Min) into @Slab_Max_SNF from m001_milk_rate_offline_mcc_slab 
		where Chart_Id = @Chart_Id and org_id = var_org_id and Slab_Type = 'SNF'  and ifnull(@Chart_Id,'')<>''  limit 1;
	...
	a.Slab_Type = 'FAT'   and ifnull(@Chart_Id,'')<>'' ;
	...
	a.Slab_Type = 'SNF'   and ifnull(@Chart_Id,'')<>'' ;
...
if(@Chart_Id is null or @Chart_Id = '')then

set @var_FinalMilkRate = null;
else
