...
DELIMITER ;;
DROP FUNCTION IF EXISTS `GetMilkRateBackDateTest` ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `GetMilkRateBackDateTest`(	
...
BEGIN
        ...
        Set @ChartId = ( select Chart_Id from f001_milk_rate where Header_Applicable_Date <= @Current_Datetime and 
        ...
        if(@MCCType = 'C014002' and var_CollectionShift_Id = 'C015003') then 
				...
				Set @ChartId = (select Chart_Id from f001_milk_rate 
                where date(Header_Applicable_Date) <=  date(@Current_Datetime) and 
				MCC_Id = var_MCC_Id and CollectionShift_Id = var_CollectionShift_Id 
				and MilkType_Id = var_MilkType_Id
			...
			if(@ChartId is null or @ChartId = '')then
				...
				Set @ChartId = (select Chart_Id from f001_milk_rate where 
                date(Header_Applicable_Date) <=  date(@Current_Datetime) and 
				MCC_Id = var_MCC_Id and CollectionShift_Id = var_CollectionShift_Id 
				and MilkType_Id = var_MilkType_Id
				order by Header_Applicable_Date desc, Item_Applicable_Date DESC limit 1 );
		...
		WHERE MCC_Id = var_MCC_Id AND MilkType_Id = var_MilkType_Id AND CollectionShift_Id = var_CollectionShift_Id
			...
			ORDER BY Item_Applicable_Date DESC,Header_Applicable_Date DESC LIMIT 1;
			...
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date  FROM f001_milk_rate where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012002' and chart_id = @ChartId 
            and date(Header_Applicable_Date) <=  date(@Current_Datetime)
            and date(Item_Applicable_Date) <=  date(@Current_Datetime)
			ORDER BY Item_Applicable_Date DESC,Header_Applicable_Date DESC
            ) slabs on 
			m014.Org_Id = slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			where Slab_Min <= var_Fat and Slab_Max >= var_Fat order by slabs.Item_Applicable_Date limit 1;

		
			select slabs.Slab_Id  into @Milk_SnfDeduction from m014_slab m014 inner join 
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date   FROM f001_milk_rate where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012003' and chart_id = @ChartId 
            and date(Header_Applicable_Date) <=  date(@Current_Datetime)
            and date(Item_Applicable_Date) <=  date(@Current_Datetime)
			ORDER BY Item_Applicable_Date DESC,Header_Applicable_Date DESC
            ) slabs on 
			m014.Org_Id = slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			where Slab_Min <= var_SNF and Slab_Max >= var_SNF order by slabs.Item_Applicable_Date desc limit 1;
            

			select slabs.Slab_Id into @Milk_Highfat from m014_slab m014 inner join 
			(SELECT Org_Id , Slab_Id ,Amount, Item_Applicable_Date FROM f001_milk_rate where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012004' and chart_id = @ChartId  
            and date(Header_Applicable_Date) <=  date(@Current_Datetime)
            and date(Item_Applicable_Date) <=  date(@Current_Datetime)
			ORDER BY Item_Applicable_Date DESC,Header_Applicable_Date DESC
            ) slabs on 
			m014.Org_Id =  slabs.Org_Id and m014.Slab_Id = slabs.Slab_Id 
			where Slab_Min <= var_Fat and Slab_Max >= var_Fat order by slabs.Item_Applicable_Date desc  limit 1;
				

			select slabs.Slab_Id into @Milk_HighSnf from m014_slab m014 inner join 
			(SELECT Org_Id , Slab_Id,Amount , Item_Applicable_Date FROM f001_milk_rate where MCC_Id = Var_MCC_Id and MilkType_Id = var_MilkType_Id
			and CollectionShift_Id = var_CollectionShift_Id and MilkRateEntryType_Id = 'C012005' and chart_id = @ChartId  
            and date(Header_Applicable_Date) <=  date(@Current_Datetime)
            and date(Item_Applicable_Date) <=  date(@Current_Datetime)
			ORDER BY Item_Applicable_Date DESC,Header_Applicable_Date DESC
            ) slabs on 
		...
		IF(var_Fat < @Milk_Base_FAT ) THEN
			if ( @Milk_FatDeduction IS NULL OR @Milk_FatDeduction in (null , '') ) then
				RETURN 0 ;
			end if;
		end if;
        	...
        	IF(var_SNF < @Milk_Base_SNF) THEN
					if( @Milk_SnfDeduction  IS NULL OR @Milk_SnfDeduction in (null , '') ) then 
						RETURN 0;
                    end if;
		end if;
        ...
        IF (var_Fat < @Milk_Base_FAT) THEN
			...
			select
			format(sum((points * 10) * Amount ), 2)  into @Milk_Fat_Deduction1
			from ( SELECT  format( if ( Slab_Min <= var_Fat  and Slab_Max >= var_Fat , (Slab_Max - var_Fat) + if (Slab_Max = @Milk_Base_FAT , 0 , 0.1) , (Slab_Max - Slab_Min ) + if (Slab_Max = @Milk_Base_FAT ,0, 0.1) ),2) as points ,
			Amount , MAX(Item_Applicable_Date)
			FROM f001_milk_rate F002 
			...
			GROUP BY F002.Slab_Id , F002.MilkType_Id , F002.CollectionShift_Id , F002.Org_Id , F002.MilkRateEntryType_Id,
				F002.MilkRateEntryType_Id , points 
		...
		ELSEIF (var_Fat > @Milk_Base_FAT) THEN
			...
			Amount , MAX(Item_Applicable_Date)
			FROM f001_milk_rate F002 LEFT JOIN 
