...
DELIMITER ;;
DROP FUNCTION IF EXISTS `GetOffline_milk_rate` ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `GetOffline_milk_rate`(
...
set @var_FinalMilkRate =  @Amount + ifnull(@total_snf_inc , 0) + ifnull(@total_fat_inc,0) - ifnull(@total_snf_ded ,0) - ifnull(@total_fat_ded , 0) ;
	...
	if(@Chart_Id is not null or @Chart_Id <>'') then

		set @var_FinalMilkRate = @var_FinalMilkRate;
